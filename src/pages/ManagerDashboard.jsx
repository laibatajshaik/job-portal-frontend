          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="space-y-1">
              <div className="flex items-center gap-1.5 text-xs text-slate-500 font-medium">
                ...
                <span>Manager</span>
                <ChevronRight className="w-3 h-3 text-slate-400" />
                <span className="text-[#FFA726] font-semibold uppercase tracking-wider">Dashboard</span>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 pt-2">
            ...
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100 flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Posted Jobs</p>
            ...
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100 flex items-center justify-between">
              ...
              <div className="text-right">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Active Roles</p>
                <p className="text-2xl font-bold text-slate-900 mt-0.5">{jobs.length}</p>
              </div>
            </div>

            {/* Card 4: Company status */}
            <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100 flex items-center justify-between">
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
            ...
            {jobs.length === 0 ? (
              <div className="text-center py-10 space-y-3">
                ...
                <p className="text-xs text-slate-505 font-medium">You haven't posted any jobs yet.</p>
                <Link
                  to="/manager/post-job"
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {jobs.map((job, index) => (
                      <tr key={index} className="hover:bg-slate-50 transition">
                        ...
                        <td className="p-3 text-slate-650">{job.location}</td>
                        <td className="p-3 font-bold text-slate-900">{formatSalary(job.salary)}</td>
                        <td className="p-3">
                          <span className="bg-slate-100 text-slate-700 px-3 py-1 rounded-full text-[11px] font-semibold border border-slate-200">
                        ...
                        <td className="p-3 text-right">
                          <Link
                            ...
                            className="inline-flex items-center gap-1 bg-[#FFA726] hover:bg-[#FB8C00] text-white px-4 py-2 rounded-full text-xs font-bold transition shadow-sm"
                          >
