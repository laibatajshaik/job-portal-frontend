  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] py-10 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        {/* Back Link */}
        <Link to="/jobs" className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-600 hover:text-[#FFA726] transition">
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-6 sm:p-8 shadow-sm space-y-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-slate-50 pb-6">
            <div>
            ...
            <span className="self-start px-4 py-1.5 rounded-full bg-[#FFA726] text-white text-xs font-bold shadow-sm">
          ...
          <div className="pt-4 border-t border-slate-100">
            {user && user.role === 'user' ? (
              <Link
                to={`/apply/${job.id}`}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-[#FFA726] hover:bg-[#FB8C00] text-white font-bold text-xs px-8 py-3.5 rounded-full shadow-md transition"
              >
                <CheckCircle2 className="w-4 h-4" />
                <span>Apply for this Position</span>
              </Link>
            ) : !user ? (
              <div className="bg-slate-50 p-4 rounded-3xl text-center text-xs text-slate-600 font-medium">
                Please{' '}
                <Link to="/login" className="text-[#FFA726] font-extrabold hover:underline">
                  Sign In
                </Link>{' '}
