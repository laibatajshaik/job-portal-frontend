    ...
    <div className="min-h-screen bg-[#F0F4F8] text-slate-800 p-4 sm:p-6 lg:p-8 font-sans">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-6">
        ...
        <aside className="w-full lg:w-64 bg-[#0B2545] rounded-3xl p-5 text-white flex flex-col justify-between shadow-xl shrink-0 min-h-[620px]">
          ...
          <div className="space-y-6">
            ...
            <nav className="space-y-1.5 text-xs font-semibold">
              ...
              <Link
                to="/jobs"
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-350 hover:text-white hover:bg-white/5 transition"
              >
        ...
        <main className="flex-1 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 pt-2">
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200/80 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Applied Jobs</p>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200/80 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-2xl font-bold text-[#0B2545] mt-0.5">
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200/80 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-2xl font-bold text-[#0B2545] mt-0.5">{shortlistedCount}</p>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200/80 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm font-bold text-[#0B2545] mt-1">Active</p>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-4">
                ...
                {applications.length === 0 ? (
                  <div className="text-center py-10 space-y-3">
                    ...
                    <Link
                      to="/jobs"
                      className="inline-flex items-center gap-1 text-xs font-bold text-[#1D3557] hover:underline"
                    >
                            ...
                            <td className="p-3">
                              <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold border ${
                                (app.status || '').toLowerCase().includes('shortlist') || (app.status || '').toLowerCase().includes('approve') || (app.status || '').toLowerCase().includes('accept')
                                  ? 'bg-[#1D3557] text-white border-none'
                      ...
                      {applications.map((app, idx) => (
                        <tr key={app.id || idx} className="hover:bg-slate-50 transition">
                          ...
                          <td className="p-3">
                            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-semibold border ${
                              (app.status || '').toLowerCase().includes('shortlist') || (app.status || '').toLowerCase().includes('approve') || (app.status || '').toLowerCase().includes('accept')
                                ? 'bg-[#1D3557] text-white border-none font-bold'
                                : (app.status || '').toLowerCase().includes('reject') || (app.status || '').toLowerCase().includes('decline')
                                ? 'bg-rose-50 text-rose-700 border-rose-100'
                                : 'bg-neutral-100 text-neutral-700 border-neutral-200'
                            }`}>
          ...
          {activeTab === 'notifications' && (
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-4">
              <h2 className="text-base font-bold text-[#0B2545]">Application Status Notifications</h2>
                  ...
                  {notifications.map((notif) => (
                    <div
                      ...
                      className={`p-4 rounded-xl border flex items-start gap-3 transition hover:shadow-sm ${
                        notif.type === 'success'
                          ? 'bg-[#0B2545] border-neutral-800 text-white shadow-md'
                          : notif.type === 'error'
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        ...
                        <span className={`block text-[10px] ${notif.type === 'success' ? 'text-slate-400' : 'text-slate-400'} font-medium`}>{notif.time}</span>
                      </div>
                    </div>
                  ))}
                </div>
